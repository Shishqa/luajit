...
restart:
  ...
  while (--ip > proto_bc(pt)) {
    ...
    if (bcmode_a(op) == BCMbase) {
      if (slot >= ra && (op != BC_KNIL || slot <= bc_d(ins))) return NULL;
...
const char *lj_debug_funcname(lua_State *L, cTValue *frame, const char **name) {
  ...
  GCfunc *fn;
  BCPos pc;
  if (frame <= tvref(L->stack) + LJ_FR2) return NULL;
...
void lj_debug_dumpstack(lua_State *L, SBuf *sb, const char *fmt, int depth) {
  ...
  while (level != depth) { /* Loop through all frame. */
    ...
    if (frame) {
      ...
      if (isluafunc(fn)) {
        GCproto *fn_proto = funcproto(fn);
        printf("%lu;", (uintptr_t)fn);
        fflush(stdout);
      } else {
        printf("@;");
      }
  ...
  printf("\n");
  fflush(stdout);
  if (lastlen) setsbufP(sb, sbufB(sb) + lastlen); /* Zap trailing separator. */
}

/* Put a compact stack dump into a buffer. */
void lj_debug_dumpstack_old(lua_State *L, SBuf *sb, const char *fmt,
                            int depth) {
  int level = 0, dir = 1, pathstrip = 1;
  MSize lastlen = 0;
  if (depth < 0) {
